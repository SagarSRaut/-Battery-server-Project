	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			try {
				...
				e.printStackTrace();
				logger.error("Database Error while Vendor Updating Details"+e);
				//response.sendRedirect("error.jsp?msg=DB error");

			}

			if (bean != null && bean.getVendor_id() > 0) {
				logger.info("Vendor Data Updated Successfully");
				response.sendRedirect("vendor_details.jsp?success=yes");
				
			} else {
				logger.info("Vendor Data is not Updated");
				response.sendRedirect("vendor_update.jsp?success=no");
			}

		} catch (Exception e) {

			try {
				logger.error("Vendor Data Not Updated try after sometimes");
				response.sendRedirect("error.jsp");
			} catch (IOException e1) {
				logger.error("IOException While Vendor Updating"+e1);
				e1.printStackTrace();
			}
		}
